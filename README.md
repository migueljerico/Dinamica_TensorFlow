...
Este repositorio es una **documentación técnica interactiva** y no incluye un despliegue web. Todo el contenido está disponible directamente en el repositorio para su consulta y replicación local. Los recursos principales son:

- **`README.md`**: Guía de inicio rápido y visión general.
- **`MANUAL_TECNICO.md`**: Documentación técnica detallada de la arquitectura y componentes.
- **`docs/Dinamica_Practica_Python_TensorFlow_Miguel_Jerico.md`**: Manual completo de la práctica.

## 📋 Descripción

Este repositorio documenta una práctica centrada en la librería **TensorFlow**, abordando su importancia en el campo de la Inteligencia Artificial y el Aprendizaje Automático. El objetivo principal fue investigar y comprender TensorFlow, pero un punto crítico y central de este documento es la **preparación y gestión del entorno Python**, crucial para el correcto funcionamiento de TensorFlow, especialmente cuando se busca aprovechar la aceleración por GPU mediante tecnologías como CUDA de NVIDIA.

El proyecto detalla el proceso y las razones técnicas detrás de la necesidad de realizar un *downgrade* en la versión de Python (de 3.14 a 3.12) para asegurar la compatibilidad con TensorFlow 2.21 y su ecosistema de aceleración por hardware. Sirve como una guía y referencia para desarrolladores y estudiantes que se enfrentan a desafíos de compatibilidad de versiones al configurar entornos de desarrollo para Machine Learning.

La documentación incluye un análisis profundo de los motivos técnicos de la incompatibilidad (desfase en ciclos de lanzamiento, dependencias C/C++, ausencia de *wheels* precompilados) y una guía paso a paso para la instalación en Windows, con especial atención a la configuración de CUDA y cuDNN para soporte GPU.

## 🖼️ Vista previa

![Vista previa](./screenshots/Infografia_TensorFlow_Miguel_Jerico.png)

*Infografía ilustrativa de TensorFlow y su ecosistema de aceleración GPU.*
...
| **Guía de *Downgrade* de Python** | Proporciona los pasos y comandos para instalar una versión compatible de Python (3.12) en sistemas Windows mediante `winget`. |
| **Arquitectura del Entorno** | Describe la pila de software necesaria (Python, TensorFlow, CUDA, cuDNN, GPU) y su interacción, incluido un diagrama de arquitectura en el manual técnico. |
| **Contexto de Aceleración GPU** | Detalla la interacción necesaria entre TensorFlow, CUDA Toolkit y cuDNN para el uso eficiente de GPUs NVIDIA. |
| **Ejemplo Conceptual de ML** | Incluye un ejemplo práctico de clasificación de dígitos con el dataset MNIST usando `tf.keras`, mostrando cómo se utiliza TensorFlow una vez configurado el entorno. |
...
1. **Instalar Python 3.12 (Windows)**  
   Dado que la práctica se centró en un problema de compatibilidad con Python 3.14 y la solución fue un *downgrade* a Python 3.12, es fundamental instalar esta versión específica. Utilice `winget` en un terminal de Windows con privilegios de administrador:
...
└── screenshots/
    └── Infografia_TensorFlow_Miguel_Jerico.png
```
...
La práctica documentada en este repositorio refleja un caso real de resolución de problemas técnicos (el *downgrade* de Python) y subraya la importancia de la gestión de entornos en el desarrollo de proyectos de Machine Learning, un conocimiento esencial para cualquier profesional del área.
